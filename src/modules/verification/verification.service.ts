...
import { EVeriification, generateOtp, ICheckOtp } from 'src/common/utils/helper/helper';
...
export class VerificationService {
    constructor(private redisService: RedisService, private prisma: PrismaService, private smsService: SmsService) { }
    ...
    async verifyOtp(payload: VerifyOtpDto) {
        ...
        if (otp !== JSON.parse(session).otp) throw new BadRequestException('invalid otp')
        await this.redisService.delete(this.getKey(type, phone))
        await this.redisService.set(
            this.getKey(type, phone, true),
            JSON.stringify(otp), 60 * 60
        )
        return {
            success: true,
            message: 'Verified'
