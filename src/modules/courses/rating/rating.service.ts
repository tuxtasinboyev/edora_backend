...
export class RatingService {
  ...
  async create(createRatingDto: CreateRatingDto, userId: number) {
    const existsCourse = await this.prisma.course.findUnique({ where: { id: createRatingDto.courseId } })
  ...
  async getRatingListBy(courseId: string, offset?: number, limit?: number) {
    const existsCourse = await this.prisma.course.findUnique({ where: { id: courseId } })
    if (!existsCourse) throw new NotFoundException("course not found!")

    const result = await this.prisma.rating.findMany({ where: { courseId }, skip: offset, take: limit })
    if (!result) throw new NotFoundException("rating not found!")
  ...
  async getCourseById(id: string) {
    const existsCourse = await this.prisma.course.findUnique({ where: { id: id } })
    if (!existsCourse) throw new NotFoundException("course not found!")

    const result = await this.prisma.rating.findMany({ where: { courseId: id } })
    if (!result) throw new NotFoundException("rating not found!")
