...
export class CourseCategoryService {
  ...
  async create(createCourseCategoryDto: CreateCourseCategoryDto) {
    const existCategory = await this.prisma.courseCategory.findUnique({ where: { name: createCourseCategoryDto.name } })
    if (existCategory) throw new ConflictException("this catory already exists!")
  ...
  async findOne(name: string) {
    ...
    const existCategory = await this.prisma.courseCategory.findUnique({ where: { name: name } })
  ...
  async update(id: number, updateCourseCategoryDto: UpdateCourseCategoryDto) {
    const existCategory = await this.prisma.courseCategory.findUnique({ where: { id: id } })
    if (!existCategory) throw new ConflictException("catgory not found!")

    const checkName = await this.prisma.courseCategory.findUnique({ where: { name: updateCourseCategoryDto.name } })
    if (checkName) throw new ConflictException("this catory already exists!")

    const result = await this.prisma.courseCategory.update({
      where: { id }, data: updateCourseCategoryDto
    })
    return {
      success: true,
      data: result
