...
export class CoursesService {
  ...
  async getCourseIdAsisant(courseId: string, offset?: number, limit?: number) {
    ...
    if (!existsCourse) throw new NotFoundException('course not found!');
    const result = await this.prisma.assignedCourse.findMany({
  ...
  async createCourseAssisent(payload: CreateAssisantDto) {
    const existsAsisants = await this.prisma.user.findUnique({
  ...
  async createCourse(
    ...
    const existsCategory = await this.prisma.courseCategory.findUnique({
  ...
  async updateCourse(
    ...
    if (
      (banner && course.banner && banner !== course.banner) ||
