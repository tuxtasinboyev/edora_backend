...
import { amountToPenny, getInMills, PaymeAccountRecord, PaymeErrorCode, PaymeErrorResponse, PaymeMethods, PaymeRequest, PaymeRequestParams, PaymeResponse, PaymeResponses, PaymeTransaction, PaymeTransactionReason, PaymeTransactionState, pennyToAmount, TAuthUser, validateWithinMinutes } from 'src/common/utils/helper/helper';
import { PaidVia, Transaction, UserRole } from '@prisma/client';
...
export class PaymentsService {
  ...
  async createPayment(payload: CheckoutDto, authUser: TAuthUser) {
  ...
  private async validateTransactionTimeout(
    ...
    if (
      !validateWithinMinutes(
        new Date(transaction.create_time ?? 0),
