...
import { AnswerDto, CreateExamDto, CreateManyExamDto, PassExamDto } from './dto/create-exam.dto/create-exam.dto';
...
export class ExamsService {
    ...
    async pass(payload: PassExamDto, userId: number) {
        ...
        if (!existsLessonGroup) throw new NotFoundException('Lesson group not found!');
    ...
    async getAllExamDetails(lessonGroupId: number) {
        ...
        const result = await this.prisma.exam.findMany({
            where: { lessonGroupId }, select: {
                id: true, question: true, variantA: true, variantB: true, variantC: true, variantD: true, answer: true, group: {
                    select: {
                        id: true,
                        name: true,
                        course: {
        ...
        if (result.length === 0) throw new NotFoundException('information not found!')
        return {
            success: true,
            data: result
        }
