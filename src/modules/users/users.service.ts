...
export class UsersService {
    ...
    async getMentorsById(id: number) {
        const existUser = await this.prisma.user.findUnique({ where: { id }, include: { mentorProfile: true } })
    ...
    async getAll(offset?: number, limit?: number, search?: string, role?: UserRole) {
        const whereClause: any = {};
        if (role) {
    ...
    async getUserPhone(phone: string) {
        const existsPhone = await this.prisma.user.findUnique({ where: { phone: phone } })
    ...
    async updateMentorProfile(id: number, payload: UpdateMentorDto) {
        ...
        const result = await this.prisma.mentorProfile.update({ where: { id }, data: payload })
        return {
            success: true,
            data: result
        }
